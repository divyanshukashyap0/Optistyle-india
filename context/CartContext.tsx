...
const CartContext = createContext<{
  ...
  addToCart: (product: Product, lens: LensOption) => void;
} | undefined>(undefined);

const cartReducer = (state: CartState, action: Action): CartState => {
  switch (action.type) {
    case 'ADD_ITEM': {
      const existing = state.items.find(item => item.id === action.payload.id && item.selectedLens?.id === action.payload.selectedLens?.id);
      let newItems;
      if (existing) {
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addToCart = (product: Product, lens: LensOption) => {
    dispatch({ 
      type: 'ADD_ITEM', 
      payload: { ...product, quantity: 1, selectedLens: lens } 
    });
  };
